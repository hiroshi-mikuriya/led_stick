...
namespace
{
    ...
    int const LED_WIDTH = 16;
    int const LED_HEIGHT = 32;
    int const LED_DEPTH = 8;

    char s_mem[LED_WIDTH][LED_HEIGHT * 3];
    std::mutex s_mutex;

    void Clear()
    {
        for(int x = 0; x < LED_WIDTH; ++x){
            std::fill(s_mem[x], s_mem[x] + LED_HEIGHT * 3, 0);
        }
    }
...
int main(int argc, const char * argv[])
{
    ...
    std::thread th([]{
        for(;;){
            short a[3] = { 0 };
            get_accel(a);
            int const line = (a[1] + 0x8000) * LED_WIDTH / 0x10000;
            s_mutex.lock();
            show_line(LED_WIDTH - 1 - line);
            s_mutex.unlock();
        }
    });
	for (;;) {
